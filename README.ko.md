...
- **친절한 에러** — 한국어 에러 메시지에 줄 번호와 그 줄의 코드가 함께 나옵니다. 이름을 틀리면 가까운 이름을 제안하고(`정의되지 않은 변수: 이릅  (혹시 '이름'?)`), 닫지 않은 `{` 는 파일 끝이 아니라 연 자리를 가리키며, 에러가 함수 안에서 나면 거기까지 온 호출 경로를 한 줄로 보여 줍니다. 미정의 변수·잘못된 인자 개수는 빌드 시점에 잡습니다
...
CI가 푸시마다 전부 실행하고, `docs/` 에 커밋된 플레이그라운드 WASM 이 지금의 `venos.cpp` 로
빌드된 것인지도 따로 확인합니다:
