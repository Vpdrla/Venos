...
- **생성 파이썬의 출력 인코딩 — stdout 만으로는 모자란다.** 윈도우 파이썬은 stdout/stderr 를 로케일 코드페이지로 인코딩해서 한글 print 가 `UnicodeEncodeError` 로 죽는다. 비-ASCII 문자열 리터럴이 있으면 `sys.stdout.reconfigure(encoding="utf-8")` 를 넣고(`sawNonAscii`), **도우미가 하나라도 붙으면 `sys.stderr` 도 같이 바꾼다** — 도우미의 예외 문구는 전부 한글이라, 출력만 고쳐 두면 **정상 실행은 멀쩡한데 에러 화면만 읽을 수 없는** 상태가 된다 (윈도우 CI 가 레슨 비교로 잡았다). **그 줄을 넣으면 `sys` 임포트도 같이** — 안 넣었더니 영어 레슨 넷이 `NameError` 로 한 줄도 못 찍고 죽었다. 이번에도 **만들어 둔 검사가 다음 실수를 잡았다**.
