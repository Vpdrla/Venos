...
- **숫자 리터럴 뒤에 점을 찍으면 파이썬에서 소수점으로 읽힌다 — 점 찍는 자리는 전부 `dotted()` 를 거칠 것.** `5.뭐()` 는 파이썬 **문법 오류**라, PyGen 이 괄호를 안 씌우면 **파싱도 안 되는 .py** 가 나간다 (틀린 답보다 나쁘다 — 학생이 손댈 데조차 없다). `dotted()` 가 `NumExpr` 앞에만 괄호를 씌운다. **세 경로가 있다**: 필드 접근(`FieldExpr`), 메서드 호출(`MethodCallExpr`), 그리고 **내장 함수의 `atom(i) + ".upper()"`** — 앞의 둘만 고쳤더니 생성 퍼저가 `upper(0)` 으로 세 번째를 찾아냈다. 그래서 `atom` 도 `dotted` 를 거치게 하나로 모았다.
