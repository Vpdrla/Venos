...
- **숫자 리터럴 뒤에 점을 찍으면 파이썬에서 소수점으로 읽힌다.** `5.뭐()` 는 파이썬 **문법 오류**라, PyGen 이 괄호를 안 씌우면 **파싱도 안 되는 .py** 가 나간다 (틀린 답보다 나쁘다 — 학생이 손댈 데조차 없다). `expr()` 의 `dotted()` 가 `NumExpr` 앞에만 괄호를 씌운다. 필드 접근(`FieldExpr`)과 메서드 호출(`MethodCallExpr`) 양쪽에 다 필요하다.
